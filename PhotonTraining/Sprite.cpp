...
#pragma endregion
	...
	/*
	SetTransform(newPosition, newScale, newRotation, addRotation);

	
	Matrix translate = Matrix::makeTranslationMatrix(transform.position.x, transform.position.y, 0.0f);
	Matrix rotate = Matrix::makeRotateMatrix(orientation, Vector(0.0f, 0.0f, transform.rotation.y));
	Matrix scale = Matrix::makeScaleMatrix(Vector(transform.scale.x, transform.scale.y, 0.0f));

	Matrix viewSpaceMatrix = translate * rotate * scale;
